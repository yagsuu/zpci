...
comptime {
    ...
    if (@hasDecl(zpci.memory, "FakeBarMemory")) {
        @compileError("zpci.memory must not expose FakeBarMemory; use zpci.testing.memory.TestBarMemory");
    }
    if (@hasDecl(zpci.testing.memory, "FakeBarMemory")) {
        @compileError("zpci.testing.memory must not expose FakeBarMemory; use TestBarMemory");
    }
}
